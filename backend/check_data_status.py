...
with db.get_session() as s:
    ...
    # 统计K线行数分布
    count_dist = {}
    for _code, count in stock_kline_counts:
        bucket = count
    ...
    print(
        f"可以计算因子的股票: {stocks_with_20plus if not isinstance(stocks_with_20plus, tuple) else stocks_with_20plus[0]} / {stock_count} ({stocks_with_20plus / stock_count * 100:.1f}%)"
    )
