...
class HistoricalDataService:
    ...
    def start_sync(
        ...
        logger.info(
            f"Starting history sync: {task.task_id}, "
            f"{start_date} to {end_date}, {len(stock_codes)} stocks"
        )
    ...
    def _sync_worker(self, task_id: str, stock_codes: List[str]):
        ...
        if failed == 0:
            ...
            logger.info(
                ...
                f"{completed} stocks, {total_klines} klines"
            )
        else:
            task.mark_failed(f"{failed} stocks failed")
            logger.warning(
                f"[{task_id}] Completed with errors: "
                f"{completed} ok, {failed} failed, {total_klines} klines"
            )
