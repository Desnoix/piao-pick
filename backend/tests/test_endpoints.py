...
This file is kept for backward compatibility - can still be run manually with:
    python tests/test_endpoints.py

For pytest, use the new test files in tests/unit/, tests/integration/, tests/e2e/
"""


def test_all_endpoints():
    """Run all endpoint tests as a single test function (for manual execution)."""
    import sys
    ...
    # Backtest (strategy "test" doesn't exist, so should be 404)
    resp = client.post(
        "/api/v1/backtest/run",
    ...
    # Selection run (strategy "test" doesn't exist, so should be 404)
    resp = client.post(
